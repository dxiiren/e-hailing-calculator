...
FILES (3 server checks)
  ...
  Browser:
    playwright.txt  -> browser automation (verify the served page on :8118) [turnkey]
  Repo (per-dev):
    github.txt      -> GitHub identity + repo/PR (PAT) [per-dev]

PRECONDITIONS
  - Tool ids are mcp__<server>__<tool> (hyphens kept).
  - playwright(localhost) needs `just start` (serves http://127.0.0.1:8118); otherwise use about:blank.
