...
| **SKIP** | not enabled for this dev, or a precondition is absent (site not being served, PAT not filled)                  |

**Bake in the known non-failures** — do not misread these as FAIL:

- **playwright (localhost:8118)** — needs `just start` running for the local page. With the server
  stopped, a connection-refused on :8118 is a SKIP, not a FAIL (playwright can retest against
  about:blank to prove the MCP itself works).
