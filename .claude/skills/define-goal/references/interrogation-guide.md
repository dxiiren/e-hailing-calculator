...
- **One theme per round.** React to each answer before moving on. If an answer is vague ("make it
  cleaner", "fix the sections"), drill in until it's checkable ("these 3 named sections", "parse
  clean + curl 200").
...
2. Is the work a **known finite list**, or must it be **discovered first**? If discovered - by what
   (grep / glob / a section scan of index.html)? Roughly how many items?
3. What proves **one** item is done? (Push for an evidence artifact - a pasted `/lint-check` parse
   line, a curl 200 line, a screenshot path - not just a feeling.)
...
- Is the target list the whole page (-> discovery sweep over its sections) or a named set?

### Migration / refactor sweep

- What's the before->after transformation, stated concretely enough to apply uniformly (e.g.
  inline `style=""` attributes -> named classes in the `<style>` block, or a palette rename)?
- Isolation: do items touch the same region of `index.html` (-> serialize) or disjoint sections
  (-> can batch)?
- What's the per-item verification (`/lint-check`'s parse / a targeted grep / a reload of :8118)
  that proves the change is safe?
...
> **No deployment bank** - this repo has no deployment target (runs locally only). A "deploy" goal
> is out of scope here; if that ever changes, add the bank back with the real environment's
> health-verify.
...
| Serve process / sanity-gate to bring up                | **Environment bootstrap**                                                                       |
