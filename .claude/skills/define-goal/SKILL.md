...
4. **Guardrails & locked decisions** - may it `git commit` / `push` / stage? May it open a PR /
   ...
   **locked** (do-not-relitigate)? Any **environment bootstrap** (`just start` on :8118, a sanity
   gate like `/lint-check`'s HTML parse)?
5. **Real-blocker definition** - what counts as a _genuine_ blocker (a code fact: a broken tag the
   parser proves, a missing asset, a hard infra failure it can't fix) versus a banned excuse ("it's
...
- Ported from akmal-resume-website - same interrogation -> confirm-until-100% -> stop-proof file
  design and the five stop-proof pillars; environment-bootstrap and evidence examples adapted from
  the Nuxt/npm/Vercel stack to this repo's `just start` / static-serve workflow.
