...
```bash
uv run python .claude/skills/audit-skills/audit.py
```
...
- Ported from akmal-resume-website unchanged in mechanism (`audit.py` verbatim);
  run command switched to `uv run python` (this repo's Python comes from uv, not a
  system install), credential-fix examples generalized for this repo.
