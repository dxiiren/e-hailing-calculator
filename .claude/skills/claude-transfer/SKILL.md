...
5. **Review in plan mode**, then on approval (create the folder if missing):
   - Write `.claude/workspace/reports/transfers/claude/{YYYY-MM-DD}-{topic}.md`, where
     `{topic}` names **what the session did** (e.g. `restyle-swot-grid`), **not** this
     skill's name.
...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   recent `git log`, then run a **basic functional check** (`just start`, then load
   `http://127.0.0.1:8118` and eyeball the page) to catch drift or undocumented breakage.
   Building new work on a silently-broken state makes it worse. Report any drift you find.
...
- Files: `index.html:120-160`, `.docs/01-overview/project-overview.md`
- Recent commits: `<hash> <subject>` (run `git log --oneline -5`)
- Branch: {branch} PR: #{id}
- Commands: `just start`, `just stop`, `just open`
...
- Server on :8118 was {up/down} at close - re-check
...
- Ported from akmal-resume-website - same pointer-based, opt-in-read design; commands +
  freshness checks adapted from the Nuxt/npm stack to this repo's `just start` / :8118
  static-serve workflow.
