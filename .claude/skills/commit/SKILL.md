...
| Changed paths                             | scope    |
| ----------------------------------------- | -------- |
| `index.html`, `app.js` (calculator UI, logic, i18n) | `app` |
...
2. **Stage by name** - `git add path/to/file ...`. **Never** `git add -A`/`.` here. Exclude
   `.env*`, secrets/tokens, `.claude/settings.local.json`, `.mcp.json`, and
   `.claude/workspace/`.
...
- This repo has **no commit hooks** (no pre-commit framework, no commitizen) - the
  Conventional Commits format is enforced by discipline, not tooling. Get it right anyway.

### Quick reference

```
<type>(<scope>): <description>
type:  feat, fix, refactor, perf, docs, style, build, chore, revert
scope: app, tooling, docs, skills, claude, mcp
```
...
- Ported from akmal-resume-website for this static single-page repo: same fast-path /
  scoped-path split and stale-lock preflight; scope table rewritten for this repo's layout;
  all pre-commit-hook machinery (gitleaks / prettier / eslint / commitizen) removed - this
  repo has no hooks.
