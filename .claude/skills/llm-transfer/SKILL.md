...
## 1. Role
You are {persona - e.g. a senior front-end engineer fluent in semantic HTML and modern CSS /
an accessibility reviewer / a technical copy editor}. {Any relevant seniority, domain, or
mindset.}
...
situation: what the project is (a single-page e-hailing driver financial calculator - Vue 3 +
vue-i18n loaded from CDNs, one index.html + app.js, no build), the domain, and WHY this task
matters.
...
## 6. Constraints & Rules
- Tech stack: {Vue 3 global build + vue-i18n 9 + Tailwind + jsPDF, all via CDN `<script>`
  tags; app logic in app.js; no build step; served by `python -m http.server` on :8118}
...
3. **Use our skills:** name the relevant playbook(s) and tell it to follow them - _"For this
   ...
   its scripts, e.g. `uv run python .claude/skills/<skill>/<x>.py`."_ Our skills are just
   markdown playbooks + stdlib Python - an agent can read and run them. **This is how the
   agent "uses our skills."**
...
Target: _"Hand off a broken-math edge case to an external LLM for a fix."_

````text
# MASTER PROMPT - Guard the required-KM math against unprofitable inputs

## 1. Role
You are a senior front-end engineer who writes precise, minimal Vue 3 fixes.

## 2. Mission
Stop the results table from showing negative "Required KM/Day" when the driver's fuel cost
per km exceeds their earnings per km, and explain the root cause in one paragraph.

## 3. Background & Context
A single-page e-hailing driver financial calculator (index.html + app.js, Vue 3 + vue-i18n
via CDN, no build). The driver enters fuel cost (RM), the km that fuel lasts, and earnings
per km; the app computes required km/day for each net-income-target x working-days combo.
No repo access - everything you need is below.

## 4. Current State
- Done: the happy path (defaults 60 / 400 / 0.7) renders correct rows.
- Failing: earningsPerKm = 0.10 with costPerKm = 0.15 yields netPerKm < 0, so
  `Math.ceil(netPerDay / netPerKm)` goes negative and the table shows "-1167 km".
...
## 6. Constraints & Rules
- Vue 3 Composition API as already used; no new dependencies, no build step.

## 7. Your Task
Return a minimal guard (skip or flag the combo when netPerKm <= 0) plus a one-paragraph
explanation of why the division flips sign.

## 8. Output Format & Success Criteria
- Output: the corrected computed block only + the explanation.
- Done: no negative KM rows; profitable inputs still produce identical numbers.

## 9. Open Questions & Assumptions
- Assume flagging the row (rather than hiding it) is acceptable if skipping loses information.

## Reminder
Return the guarded computed block + a one-paragraph root cause.
````
