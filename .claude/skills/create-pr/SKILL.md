...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `feat(app): add EV cost-per-km mode` or
  `docs: expand the troubleshooting guide`. Pick the type/scope from the dominant
  change, not just the first commit.
...
- <bullet per meaningful change — calculator UI / logic / i18n / docs / tooling touched>
- <...>

## Testing

- <how it was verified: `just start` + reload http://127.0.0.1:8118 and describe what
  was checked, or "docs-only — rendered preview checked">
```
...
title used. This repo has no CI — there are no checks to wait for; the PR is
review-ready immediately.
...
| `/pre-pr-review` | Optional self-review (HTML/CSS/a11y checklist) before opening the PR          |
