...
| Serving | `python -m http.server` via uv | `just start` on 127.0.0.1:8118; no build step, no backend, no package.json |
...
- The math has no unprofitability guard: if fuel cost per km >= earnings per km, `requiredKM`
  goes negative (see `.docs/06-troubleshooting/common-issues.md`).
