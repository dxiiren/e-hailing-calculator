...
/**
 * True when driving cannot net anything: fuel cost per km is at or above
 * earnings per km (this also covers fuelKm 0 -> Infinite cost, and NaN from
 * blank inputs, since NaN comparisons make netPerKm > 0 false).
 */
export function isUnprofitable({ fuelCost, fuelKm, earningsPerKm }) {
  const netPerKm = earningsPerKm - computeCostPerKm({ fuelCost, fuelKm });
  return !(netPerKm > 0);
}
 ...
 *
 * When fuel cost per km >= earnings per km no distance can reach the target,
 * so the combo is returned with `unprofitable: true` and null figures instead
 * of a negative (or Infinite) requiredKM.
 *
 * @param {object} input
...
export function computeRequirement({ income, days, fuelCost, fuelKm, earningsPerKm }) {
  ...
  if (isUnprofitable({ fuelCost, fuelKm, earningsPerKm })) {
    return {
      income,
      days,
      netPerDay,
