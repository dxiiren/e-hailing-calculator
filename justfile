# E-Hailing Financial Calculator justfile — development recipes

set shell := ["powershell.exe", "-NoProfile", "-Command"]

port := env_var_or_default('PORT', '8118')
...
start: _require-uv stop
    Start-Process powershell -ArgumentList "-NoProfile", "-Command", "uv run python -m http.server {{port}} --bind 127.0.0.1 --directory '{{justfile_directory()}}\.'"
    Start-Sleep -Seconds 2
    Write-Host "Started: http://127.0.0.1:{{port}}  (stop with: just stop)"

# Serve in the FOREGROUND (Ctrl+C to stop).
serve: _require-uv
    uv run python -m http.server {{port}} --bind 127.0.0.1 --directory '{{justfile_directory()}}\.'
